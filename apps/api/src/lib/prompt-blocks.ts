...
export function joinPromptSections(
  sections: ReadonlyArray<string | readonly string[] | undefined>,
): string {
  const lines: string[] = []
...
export function successCriteriaBlock(lines: readonly string[]): string {
...
export function verificationBlock(lines: readonly string[]): string {
...
export function stopRulesBlock(lines: readonly string[]): string {
...
export function formattingBlock(lines: readonly string[]): string {
  return `# Format\n${lines.map((line) => `- ${line}`).join('\n')}`
}

export function outputBlock(lines: readonly string[]): string {
