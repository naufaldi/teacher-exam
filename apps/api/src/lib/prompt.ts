...
function buildBahasaInggrisRules(): readonly string[] {
  return [
...
function buildQuestionShapeOutputLines(totalSoal: number): readonly string[] {
  return [
...
export function buildExamPrompt(input: BuildPromptInput): BuiltPrompt {
  ...
  const bahasaInggrisRules =
    input.examSubject === 'bahasa_inggris' ? buildBahasaInggrisRules() : []
...
export function buildRegeneratePrompt(input: BuildRegeneratePromptInput): BuiltPrompt {
  ...
  const corpusSections =
    input.curriculumText && input.curriculumText.trim() !== ''
