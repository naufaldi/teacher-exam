...
describe("buildPembahasanPrompt", () => {
  ...
  it("user contains all 20 questions serialized as JSON", () => {
    ...
    expect((parsed as Array<unknown>).length).toBe(20)
  })
  ...
  it("throws when questions array is empty", () => {
    expect(() => buildPembahasanPrompt({ exam: FAKE_EXAM, questions: [] })).toThrow(
      "buildPembahasanPrompt: questions must not be empty"
    )
  })
