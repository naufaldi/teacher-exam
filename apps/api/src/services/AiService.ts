...
export interface AiService {
  ...
  validateCurriculum: (
    input: ValidateCurriculumInput
  ) => Effect.Effect<ReadonlyArray<CurriculumValidationItem>, AiGenerationError>
...
function callGenerateText(
  ...
  const model = slot === "discussion" || slot === "validation"
...
export function createAiService(config: AiServiceConfig): AiService {
  ...
  return {
    generateRaw(input) {
      ...
      const promptInput = input.pdfBytes !== undefined
      ...
      return callGenerateObject(
        ...
        Effect.catchAll(() => callGenerateText(config, models, promptInput, slot))
      )
    },

    generate({ expectedCount, pdfBytes, system, user }) {
      const slot = pdfBytes !== undefined ? "pdf" : "text"
      const generateModel = slot === "pdf" ? models.pdfModel : models.generateModel
      const promptInput = pdfBytes !== undefined ? { system, user, pdfBytes } : { system, user }
    ...
    validateCurriculum({ expectedCount, system, user }) {
      return Effect.gen(function*() {
        const batch = yield* callGenerateObject(
          config,
          models,
