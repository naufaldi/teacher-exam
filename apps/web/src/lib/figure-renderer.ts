...
function text(x: number, y: number, label: string, anchor = 'middle'): string {
  return `<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="12" font-family="Plus Jakarta Sans, Arial, sans-serif" fill="#111827">${escapeSvgText(label)}</text>`
}
  ...
  switch (spec.type) {
    ...
    case 'triangle':
      return frame([
        ...
        spec.label ? text(120, 148, spec.label) : '',
      ].join(''))
    case 'trapezoid':
      return frame([
...
function renderCoordinatePlane(spec: Extract<FigureSpec, { type: 'coordinate_plane' }>): string {
  const left = 36
  const right = 216
  const top = 16
  const bottom = 136
  ...
  const points = spec.points.map((p) => {
    ...
    return [
      ...
      p.label ? text(x + 8, y - 8, p.label, 'start') : '',
    ].join('')
  }).join('')
  ...
  return frame([
    ...
    points,
    spec.label ? text(120, 154, spec.label) : '',
  ].join(''))
}
