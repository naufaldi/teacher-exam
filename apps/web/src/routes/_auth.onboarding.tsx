...
const ALL_GRADES: ReadonlyArray<Grade> = [1, 2, 3, 4, 5, 6] as const
const DEFAULT_GRADES: Array<Grade> = [5, 6]
...
function OnboardingPage() {
  ...
  const [grades, setGrades] = useState<Array<Grade>>(DEFAULT_GRADES)
  const [subjects, setSubjects] = useState<Array<ExamSubject>>(["bahasa_indonesia"])
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          {error ?
            (
              <div
                role="alert"
                className="rounded-sm border border-amber-200 bg-amber-50 px-3 py-2 text-body-sm text-amber-800"
              >
                {error}
              </div>
            ) :
