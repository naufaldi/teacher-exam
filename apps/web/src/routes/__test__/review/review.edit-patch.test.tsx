...
import { makeMcqMulti } from "../../../test/fixtures/exam.js"
import { makeExamWithMixedTypes, makeExamWithQuestions } from "./fixtures.js"
import {
  getLoader,
...
describe("ReviewPage — Edit dialog saves via api.questions.patch", () => {
  it("reverts question text in store when api.questions.patch rejects", async () => {
    ...
    const patchSpy = mockQuestionsPatch.mockRejectedValue(new Error("Network error"))
...
describe("ReviewPage — Task 13: Edit dialog save fires api.questions.patch with typed payload", () => {
  it("mcq_multi edit save sends {_tag:\"mcq_multi\", correct:[...]} to patch", async () => {
    ...
    mockApiResolvedValueOnce(mockExamsGet, makeExamWithMixedTypes("exam_multi_edit"))
    await getLoader()({ deps: { examId: "exam_multi_edit" } })

    const patchSpy = mockApiSpyResolvedValue(mockQuestionsPatch, {
      ...makeMcqMulti(1, ["a", "c"], { examId: "exam_multi_edit" }),
